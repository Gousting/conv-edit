...
> 2026-07-10 · v6→v7: 剪辑风格预设 + LLM智能选片 + VLM视觉审查
...
### 2.2 半自动（LLM 辅助选片 + BGM 感知）
```
丢视频 → (可选)上传BGM → 🤖智能选片(VLM看图+BGM节奏挑片)
→ 你确认/微调 → (若未传BGM则此时上传) → 🎯生成时间线
→ 🤖LLM审查 → 🚀渲染下载
```
先传 BGM 再选片：LLM 会根据 BPM/段落结构决定选哪些片段。
例如快节奏 BGM(140BPM)→优先短切高能片段；慢节奏 BGM→保留长镜头。
